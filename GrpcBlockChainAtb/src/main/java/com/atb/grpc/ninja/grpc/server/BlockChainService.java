...
public class BlockChainService extends BlockChainServiceImplBase { 
   ...
   private BlockDataDao dao;
		      ...
		      StreamObserver<BlockConfirm> responseObserver)
	{
		...
		if (request.getLinkHash().equals(encriptSrv.dummyLinkHashDecode())) {
			log.info("@@@ ATB @@@ - Una vez comprobada la validez del bloque en el servidor, la guardamos en Mongo ");
			...
			log.info("@@@ ATB @@@ - Retornamos el hash del nuevo bloque, clave para la vinculación del siguiente");
